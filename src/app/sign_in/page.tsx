...
const SignInForm: FC = () => {
  ...
  const [passConfValue, setPassConfValue] = useState<string>('');
  const [isPassConfTouched, setIsPassConfTouched] = useState<boolean>(false);
  ...
  const isPassConfValid = passConfValue.trim().length > 0;
  const isWrongPassConf = passConfValue.trim().length < 8;
  const hasPassConfError =
    (!isPassConfValid && isPassConfTouched) ||
    (isWrongPassConf && isPassConfTouched);
  ...
  const passConfClass = `w-full rounded-md pl-10 text-sm ${
    hasPassConfError
      ? 'bg-red-100'
      : 'border-gray-300 focus:border-green-500 focus:ring-blue-500 bg-cyan-200'
  }`;
  ...
  useEffect(() => {
    if (
      ...
      !hasPassError &&
      !hasPassConfError &&
      passValue.trim() === passConfValue.trim()
    ) {
      setLoading(true);
      (async () => {
        try {
          const data = await logIn({ loginValue, passValue, action: 'signIn' });
          ...
          doSteps(labelsErrorModal.signInError);
        } finally {
          setLoading(false);
        }
      })();
    ...
    if (
      ...
      doSteps(labelsErrorModal.wrongPass);
    }
  }, [createNewUser]);
          ...
          <div className="mx-auto mt-6 w-full max-w-md rounded-xl bg-white/80 p-6 shadow-xl backdrop-blur-xl sm:mt-10 sm:p-10 space-y-6">
            ...
            <div>
              ...
              <div className="relative mt-1 rounded-md shadow-sm">
                ...
                <input
                  ...
                  className={passConfClass}
                  onChange={e => setPassConfValue(e.target.value)}
                  onBlur={() => setIsPassConfTouched(true)}
                  placeholder="Minimum 8 characters"
                />
              </div>
              {hasPassConfError && !isPassConfValid && (
                <div className="flex justify-center">
                  <p className="mt-1 text-sm text-red-500">
                    Password is required.
                  </p>
              ...
              {hasPassConfError && isPassConfValid && isWrongPassConf && (
                <div className="flex justify-center">
                  <p className="mt-1 text-sm text-red-500">
                    Minimum 8 characters.
                  </p>
            ...
            <div>
              {loading ? (
                <button
                  type="button"
                  className="flex items-center justify-center rounded-md w-full text-white bg-white-600 py-2 px-4 bg-green-600 font-semibold shadow-lg transition duration-150 ease-in-out hover:bg-green-700 hover:shadow-xl focus:shadow-xl focus:outline-none focus:ring-2 focus:ring-green-500 focus:ring-offset-2 disabled"
                >
